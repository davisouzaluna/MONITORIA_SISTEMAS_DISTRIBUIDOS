#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

// Declarando dois mutexes
pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex2 = PTHREAD_MUTEX_INITIALIZER;

// Função que tenta obter os dois mutexes em ordem inversa, causando deadlock
void *deadlock_thread1(void *arg) {
    printf("Thread 1: Tentando bloquear mutex1...\n");
    pthread_mutex_lock(&mutex1);
    printf("Thread 1: Mutex1 bloqueado\n");

    // Simula algum trabalho(dormir tbm eh um trabalho :/)
    sleep(1);

    printf("Thread 1: Tentando bloquear mutex2...\n");
    pthread_mutex_lock(&mutex2);
    printf("Thread 1: Mutex2 bloqueado\n");

    // Realiza alguma operação (não será alcançada devido ao deadlock)
    pthread_mutex_unlock(&mutex2);
    pthread_mutex_unlock(&mutex1);

    return NULL;
}

void *deadlock_thread2(void *arg) {
    printf("Thread 2: Tentando bloquear mutex2...\n");
    pthread_mutex_lock(&mutex2);
    printf("Thread 2: Mutex2 bloqueado\n");

    // Simula algum trabalho
    sleep(1);

    printf("Thread 2: Tentando bloquear mutex1...\n");
    pthread_mutex_lock(&mutex1);
    printf("Thread 2: Mutex1 bloqueado\n");

    // Realiza alguma operacao (nao sera alcançada devido ao deadlock)
    pthread_mutex_unlock(&mutex1);
    pthread_mutex_unlock(&mutex2);

    return NULL;
}

int main() {
    pthread_t thread1, thread2;

    // Criando duas threads que causam deadlock
    pthread_create(&thread1, NULL, deadlock_thread1, NULL);
    pthread_create(&thread2, NULL, deadlock_thread2, NULL);

    // Aguardando ambas as threads (isso pode demorar indefinidamente devido ao deadlock)
    pthread_join(thread1, NULL);
    pthread_join(thread2, NULL);

    return 0;
}
